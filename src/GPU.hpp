...
class GPU {
   public:
    ...
    static constexpr Area<float> circle_area{circle_radious * 2,
                                             circle_radious * 2};
   ...
   private:
    ...
    struct Widget {
        static inline const cairo::Image ico_gpu{
            resources / "icons/Computer/004-video-card.png", theme::green};
        static inline const cairo::Image ico_mem{
            resources / "icons/Computer/017-processor.png", theme::white};
        static inline const cairo::Image ico_temp{
            resources / "icons/Nature/049-thermometer.png", theme::red};
        static inline const cairo::Image ico_fan{
            resources / "icons/Computer/054-cooler.png", theme::blue};

        Widget(Window& w, const Device& d, Position<float> pos,
               Area<float> area)
            : d{d},
              list{w, pos.offset({0, circle_area.h}),
                   proc_line.scale({1, max_procs + 1})} {
            using namespace ::std::numbers;

            const auto center{pos.offset({circle_radious, circle_radious})};
            const Position<float> outer_edge{(circle_radious + 2) / sqrt2,
                                             (circle_radious + 2) / sqrt2};
            const auto value_label{
                theme::medium_area(circle_radious - outer_edge.x)};
            const Position<float> inner_edge{(circle_radious - 18) / sqrt2,
                                             (circle_radious - 18) / sqrt2};
            ...
            tb.pos = value_label.bottom_right(
                center.offset(outer_edge.scale({-1, -1})));
            ...
            w.draw(
                ico_fan,
                icon_size.bottom_right(center.offset(inner_edge.scale({1, 1}))),
                icon_size);
        }
        const Device& d;
   ...
   public:
    GPU(Position<int> pos)
        : pos{pos}, nvml{}, devices{nvml.get_devices<max_procs>()} {}
    ...
    void draw(Window& w, bool new_data) {
        for (auto& widget : widgets) {
            widget.usage.draw(w);
            widget.usage_percent.draw(
                w, ftos<0>(widget.usage.current_percentage()) + "%");
            widget.freq.draw(w,
                             width<4>(to_string(widget.freqv.draw())) + "MHz");

            widget.memory_usage.draw(w);
            widget.memory_usage_percent.draw(
                w, ftos<0>(widget.memory_usage.current_percentage()) + "%");
            widget.mem_usage.draw(w, width<5>(ftos<3>(widget.memv.draw())) +
                                         "/" + ftos<0>(widget.d.memory_total) +
                                         "GB");

            widget.temp.draw(w);
            widget.temp_celsius.draw(
                w, ftos<0>(widget.temp.current_percentage()) + "℃");
            widget.watts.draw(w, ftos<1>(widget.wattsv.draw()) + "W");

            widget.fan.draw(w);
            widget.fan_percent.draw(
                w, ftos<0>(widget.fan.current_percentage()) + "%");
            widget.list.draw(w);
        }
    }
    ...
    [[nodiscard]] Window create_window() {
        Window w{":0.0", pos,
                 Area<float>{circle_area.w * devices.size(),
                             circle_area.h + theme::small_h * (max_procs + 1)}};
        ...
        widgets = [&] {
            ...
            for (auto [idx, d] : devices | enumerate) {
                ...
                temp.push_back(
                    {w, d, rpos,
                     Area<float>{
                         circle_area.w * devices.size(),
                         circle_area.h + proc_line.h * (max_procs + 1)}});
            }
            return temp;
        }();
